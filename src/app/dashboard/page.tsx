  ...
  return (
    <PortalShell title={`Good afternoon, ${profile.fullName.split(" ")[0]}`} description={`${friendlyDate(isoDate(), "long")} · Here’s what’s cooking.`}>
      <section className="dashboard-welcome">
        <div><span><Sparkles size={15} /> Today at CVmess</span><h2>Three fresh meals.<br />Order when you’re ready.</h2><p>Every confirmed order is added to your monthly bill automatically.</p></div>
      ...
      <section className="stats-grid member-stats">
        <StatCard label="August bill" value={formatMoney(monthTotal)} helper={`${confirmed.length} confirmed meals`} icon={ReceiptText} tone="olive" />
      ...
      <section className="meal-grid">
        {todayMenu.map((item) => <MealCard key={item.id} item={item} />)}
      </section>
