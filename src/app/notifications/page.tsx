  ...
  return (
    <PortalShell title="Notifications" description="Order decisions, menu updates, and billing news.">
      <section className="panel notifications-panel">
        ...
        {notifications.length ? <div className="notification-list">{notifications.map((note) => { const Icon = icons[note.type]; return <button key={note.id} className={`notification-row ${note.isRead ? "" : "unread"}`} onClick={() => void markNotificationRead(note.id)}><span className={`notification-icon note-${note.type}`}><Icon size={19} /></span><div><strong>{note.title}</strong><p>{note.message}</p><small>{timeAgo(note.createdAt)}</small></div>{!note.isRead && <i />}</button>; })}</div> : <EmptyState title="You’re all caught up" text="Order confirmations and bill updates will appear here." />}
      </section>
    </PortalShell>
  );
}
